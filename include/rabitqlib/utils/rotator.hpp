...
namespace rabitqlib
{
    ...
    namespace rotator_impl
    {
        ...
        {
            // Use AVX2 for Lambda compatibility (no AVX512 support)
            constexpr size_t kFloatsPerChunk = 32; // Process 32 floats per iteration (4×8)

            static_assert(
                kFloatsPerChunk % 8 == 0,
                "floats_per_chunk must be divisible by AVX2 register width");

            for (size_t i = 0; i < dim; i += kFloatsPerChunk)
            {
                // Load 32 bits (4 bytes) from the bit sequence
                uint32_t mask_bits;
                std::memcpy(&mask_bits, &flip[i / 8], sizeof(mask_bits));

                // Prepare sign-flip constant for AVX2
                const __m256 sign_flip = _mm256_castsi256_ps(_mm256_set1_epi32(0x80000000));

                // Process 8 floats at a time (AVX2 width)
                for (size_t j = 0; j < 4; ++j)
                {
                    // Extract 8-bit mask for current chunk
                    uint32_t chunk_mask = (mask_bits >> (j * 8)) & 0xFF;

                    // Load 8 floats
                    __m256 vec = _mm256_loadu_ps(&data[i + j * 8]);

                    // Apply conditional sign flip based on mask bits
                    // Create a mask vector from individual bits
                    ...
                    // Convert to float mask and apply XOR
                    __m256 mask_ps = _mm256_castsi256_ps(mask_vec);
                    __m256 xor_val = _mm256_and_ps(sign_flip, mask_ps);
                    vec = _mm256_xor_ps(vec, xor_val);
