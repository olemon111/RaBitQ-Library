...
namespace rabitqlib::symqg
{
    ...
    private:
        ...
        // Helper function to read page fault statistics from /proc/self/stat
        // Returns pair<minflt, majflt>
        static std::pair<unsigned long, unsigned long> get_page_fault_stats()
        {
#ifdef __linux__
            ...
            // Parse the stat file: format is "pid (comm) state ppid ... minflt ... majflt ..."
            // Field 1: pid
            // Field 2: comm (enclosed in parentheses, may contain spaces)
            // Field 3: state
            // ...
            // Field 10: minflt - minor page faults
            // Field 11: cminflt
            // Field 12: majflt - major page faults
            // Strategy: find the closing ')' of comm field, then parse fields from there
            // After ')', the next token is field 3 (state), so we need 7 more tokens to reach field 10
            ...
            // Start parsing after the comm field (after the closing ')')
            std::istringstream iss(line.substr(comm_end + 1));
            std::string token;
            unsigned long minflt = 0, majflt = 0;
            int field_idx = 3; // We've already passed pid (field 1) and comm (field 2)

            while (iss >> token && field_idx <= 12)
            {
                if (field_idx == 10)
                {
                    minflt = std::stoul(token);
                }
                else if (field_idx == 12)
                {
                    majflt = std::stoul(token);
                    break;
                }
                field_idx++;
            }
...
/* Data via mmap */
    ...
    {
        // Record page fault statistics before search
        auto [minflt_before, majflt_before] = get_page_fault_stats();
        ...
        // // Record page fault statistics after search and print
        // auto [minflt_after, majflt_after] = get_page_fault_stats();
        // unsigned long minflt_delta = (minflt_after >= minflt_before) ? (minflt_after - minflt_before) : 0;
        // unsigned long majflt_delta = (majflt_after >= majflt_before) ? (majflt_after - majflt_before) : 0;

        // std::cout << "[PageFaultStats] Minor faults: " << minflt_delta
        //           << ", Major faults: " << majflt_delta
        //           << " (total: " << (minflt_delta + majflt_delta)
        //           << ", before: min=" << minflt_before << " maj=" << majflt_before
        //           << ", after: min=" << minflt_after << " maj=" << majflt_after << ")" << std::endl;
    }
    ...
    inline void QuantizedGraph<T>::initialize()
    {
        ...
        this->neighbor_offset_ = batch_data_offset_ + QGBatchDataMap<T>::data_bytes(padded_dim_) * (degree_bound_ / fastscan::kBatchSize);
