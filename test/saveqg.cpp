...
int main()
{
    ...
    auto fp_ptr = (uint64_t *)(ep_ptr + 1);
    std::cout << "index_size: " << *index_size_ptr
              << ", max_od: " << *max_od_ptr
              << ", ep: " << *ep_ptr
              << ", fp: " << *fp_ptr << std::endl;

    const size_t M = *max_od_ptr;
    auto graph_ptr = (uint32_t *)(fp_ptr + 1);
    expect_eq(diskann_mmap.size_ - 24, ds.nvecs() * (M + 1) * sizeof(uint32_t));

    rabitqlib::symqg::QuantizedGraph<> qg(ds.nvecs(), ds.dim(), M);

    qg.copy_vectors_public(ds.vecs());
    qg.set_ep(*ep_ptr);

#pragma omp parallel
    {
        std::vector<rabitqlib::AnnCandidate<float>> nbrs(M);
#pragma omp for
        for (size_t i = 0; i < ds.nvecs(); ++i)
        {
            uint32_t *nbr = graph_ptr + i * (M + 1) + 1;
            for (size_t j = 0; j < M; ++j)
            {
                nbrs[j].id = nbr[j];
                nbrs[j].distance = 0;
            }
            qg.update_qg_public(i, nbrs);
        }
    }
    ...
    if (mkdir("output", 0755) != 0)
    {
        if (errno != EEXIST)
        {
            std::cerr << "Failed to create output directory: " << std::strerror(errno) << std::endl;
            return 1;
        }
    }

    qg.save_index("output/qg_metadata.bin", "output/qg_data.bin");
